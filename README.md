...
- `src/` → code for audit, preprocessing, training, evaluation
    - `tools.py` crossfunctional code distributed across multiple files from a dedicated silo
...
(4/15/26) With a significant divergence between kaggle score and predicted accuracy, I spent several more hours refining my tuning and feature engingeering.\
Focus shifted to hyperparameter tuning via `GridSearchCV` to find the optimal `RandomForest` configuration. I conducted t5wo rounds of grid search to find optimal parameters.
...
Even though 0.3 may be more appropriate for real-world applications , i.e. using a similar model to attempt to contact shipwreck survivors, 0.45 is the correct submission threshold for competitive accuracy. 
